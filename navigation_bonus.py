...
class GapBarrier:
    def __init__(self):
        ...
        self.roi_u_lower = rospy.get_param('~roi_u_lower')
        self.roi_u_upper = rospy.get_param('~roi_u_upper')
        self.roi_v_lower = rospy.get_param('~roi_v_lower')
        self.roi_v_upper = rospy.get_param('~roi_v_upper')
        ...
        self.cam_ranges     = None  # Valid scanned ranges
        self.cam_angles     = None  # Valid scanned angles 
    ...
    def imageDepthCallback(self, data):
        ...
        try:
            ...
            if self.intrinsics:
                ...
                mask = (y < self.y_cam_max) & (y > self.y_cam_min) & (z > 0) & (z < self.max_depth) & (x<self.x_cam_max) & (x>self.x_cam_min)
                x = x[mask]
                #y = y[mask] # we don't need the y component anymore
                z = z[mask]
                ...
                self.cam_ranges = np.sqrt((x)**2 + (z)**2)  # Euclidean distance
                self.cam_angles = np.arctan2(z, x)          # Angle in radians
