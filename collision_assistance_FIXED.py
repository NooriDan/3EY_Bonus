...
class CollisionAssistance:
    ...
    def obst_idnt(self,ls_ranges):
        ...
        for i in range(nm_obs):  # replaced part
    ...
    def ptn_fld(self, nm_obs,arg_ls_obs,ls_ranges):
        ...
        for i in range(nm_obs):
            ...
            f_rep_x=f_rep_x+self.f_gain*(1-self.d_obs/d)*math.cos(theta) # replaced
            f_rep_y=f_rep_y+self.f_gain*(1-self.d_obs/d)*math.sin(theta) # replaced
    ...
    def lidar_callback(self, data):
        ...
        # 
        vel_d = self.etha_vel*self.vel_joy + (1-self.etha_vel)*self.vel # replaced code

        if self.vel_joy >=0 and vel_d < 0:
            vel_d = 0

        u0 = self.etha_delta*math.tan(self.steer_joy)+ (1 - self.etha_delta) * (self.wheelbase/max(vel_d**2, 0.0001))* f_tot_y # replaced code
